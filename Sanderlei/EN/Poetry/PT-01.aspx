<%@ Page Title="Luís Vaz de Camões - Amor é fogo que arde sem se ver" Language="C#" MasterPageFile="~/EN/SitePTPoesia.master" AutoEventWireup="true" %>
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder11" runat="server">
    <meta name="description" content=" Amor é fogo que arde sem se ver, é ferida que dói, e não se sente; é um contentamento descontente, é dor que desatina sem doer. " /> 
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder12" runat="server">
      <!-- Conteudo  --> 
      <div class='container panel-body'>
        <div class='col-xs-12 col-md-12 text-center'>
            <br>  <h1> Luís Vaz de Camões - Amor é fogo que arde sem se ver </h1>  <br> <br> 
              <div class="embed-responsive embed-responsive-16by9">
                 <iframe class="embed-responsive-item" src="https://www.youtube.com/embed/9chlCY_6xMQ" frameborder="0" allowfullscreen></iframe>  </div>  <br> 
            <div class='text-center' style='line-height:1.7'> <Br />  <p> </p>  
            <h3>    Luís Vaz de Camões - Amor é fogo que arde sem se ver</h3>
                <Br /> 
 <p> Amor é fogo que arde sem se ver,</p>        
 <p> é ferida que dói, e não se sente;</p>        
 <p> é um contentamento descontente,</p>        
 <p> é dor que desatina sem doer.</p>        
<Br /> 
 <p> É um não querer mais que bem querer;</p>        
 <p> é um andar solitário entre a gente;</p>        
 <p> é nunca contentar-se de contente;</p>        
 <p> é um cuidar que ganha em se perder.</p>        
<Br /> 
 <p> É querer estar preso por vontade;</p>        
 <p> é servir a quem vence, o vencedor;</p>        
 <p> é ter com quem nos mata, lealdade.</p>        
<Br /> 
 <p> Mas como causar pode seu favor</p>        
 <p> nos corações humanos amizade,</p>        
 <p> se tão contrário a si é o mesmo Amor?</p>        
<Br /> 

<h4>Luís Vaz de Camões (1524—1580) foi um poeta de Portugal, </h4>
<h4>considerado uma das maiores figuras da literatura em língua portuguesa</h4>
<h4>e um dos grandes poetas do Ocidente.</h4>
            </div>  <br> <br>   <br>  <br>  <br> <br>
			<br>
        </div>
      </div>
</asp:Content>