<%@ Page Title="Raimundo Correia - As Pombas" Language="C#" MasterPageFile="~/EN/SitePTPoesia.master" AutoEventWireup="true"  %>
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder11" runat="server">
    <meta name="description" content="Vai-se a primeira pomba despertada...Vai-se outra mais... mais outra... enfim dezenas. De pombas vão-se dos pombais, apenas Raia sanguínea e fresca a madrugada... " /> 
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder12" runat="server">
      <!-- Conteudo  --> 
      <div class='container panel-body'>
        <div class='col-xs-12 col-md-12 text-center'>
            <br>  <h1>Raimundo Correia - As Pombas</h1>  <br> <br> 
              <div class="embed-responsive embed-responsive-16by9">
                 <iframe class="embed-responsive-item" src="https://www.youtube.com/embed/3p-oraOhWGk" frameborder="0" allowfullscreen></iframe>  </div>  <br> 
            <div class='text-center' style='line-height:1.7'> <Br />  <p> </p> 
               <h3> Raimundo Correia - As Pombas</h3>
                <Br /> 
<p>Vai-se a primeira pomba despertada...</p>          
<p>Vai-se outra mais... mais outra... enfim dezenas</p>          
<p>De pombas vão-se dos pombais, apenas</p>          
<p>Raia sanguínea e fresca a madrugada...</p>          
<Br /> 
<p>E à tarde, quando a rígida nortada</p>          
<p>Sopra, aos pombais de novo elas, serenas,</p>          
<p>Ruflando as asas, sacudindo as penas,</p>          
<p>Voltam todas em bando e em revoada...</p>          
<Br /> 
<p>Também dos corações onde abotoam,</p>          
<p>Os sonhos, um por um, céleres voam,</p>          
<p>Como voam as pombas dos pombais;</p>          
<Br /> 
<p>No azul da adolescência as asas soltam,</p>          
<p>Fogem... Mas aos pombais as pombas voltam,</p>          
<p>E eles aos corações não voltam mais...</p>          
<Br /> 

<h4>Raymundo da Motta de Azevedo Corrêa (1859-1911) </h4>
<h4>foi um juiz e poeta brasileiro.</h4>


            </div>  <br> <br>   <br>  <br>  <br> <br>
			<br>
        </div>
      </div>
</asp:Content>
